module.exports = function(async, Users, Message, FriendResult){
    return {
        PrivateMessaging: function(router){
        ...
        getchatPage: function(req, res){
            async.parallel([
                ...
                res.render('private/privatechat', {title: 'Social chat', user:req.user, data: result1, chat: result2, chats:result3, name:nameParams});
            });
        },
        ...
        chatPostPage: function(req, res, next){
            ...
            console.log("printing nameRegex ",nameRegex);
            
            async.waterfall([
                function(callback){
                    if(req.body.message){
                        Users.findOne({'username':{$regex: nameRegex}}, (err, data) => {
                            console.log("inside privatechat 1st chatPostPage ",data);
                           callback(err, data);
                        });
                    }
                },
                ...
                function(data, callback){
                    //console.log("inside privatechat 2nd chatPostPage ",data);
                    if(req.body.message){
